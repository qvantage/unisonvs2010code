...
Public Class smPricePlanCustomer
    Inherits System.Windows.Forms.Form

    Dim SqlSelect As String = "Select c.Name, c.CustomerId, ppc.PlanID From " & smBILLTblPath & "Customer c, " & smBILLTblPath & "PricePlanCustomer ppc where c.CustomerID=ppc.CustomerID"
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.Name = "smPricePlanCustomer"
        Me.Tag = "Customer"
        Me.Text = "Settlement Price Plan Customer"
...
#End Region
    ...
    Private Sub smPricePlanCustomer_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.CenterToScreen()
    ...
    Private Sub LoadData()
        ...
        Dim SqlSelectUG2 As String = "Select Convert(bit, 0) as CHK, pp.PlanID, pp.Plan_Name, pp.Start_Date, pp.End_Date From " & smBILLTblPath & "PricePlans pp, " & smBILLTblPath & "PricePlanCustomer ppc, " & smBILLTblPath & "Customer c where pp.PlanID=ppc.PlanID and ppc.CustomerID = c.CustomerID and ppc.CustomerID = '" & utID.Text & "'"
        ...
        If SelPlans = "(" Then
            ...
            SqlSelectUG1 = "Select Convert(bit, 0) as CHK, pp.PlanID, pp.Plan_Name, pp.Start_Date, pp.End_Date From " & smBILLTblPath & "PricePlans pp"
        Else
            SelPlans = SelPlans.Substring(0, Len(SelPlans) - 2) & ")"
            SqlSelectUG1 = "Select Convert(bit, 0) as CHK, pp.PlanID, pp.Plan_Name, pp.Start_Date, pp.End_Date From " & smBILLTblPath & "PricePlans pp where pp.PlanID not in " & SelPlans & ""
    ...
    Private Sub btnSelectName_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSelectName.Click, btnSelectID.Click
        ...
        If btnSelectName.Focused = True Then
            SQLSelName = "Select Name, CustomerID, Address1, Address2, City, State, Zip, Contact, Phone, CourierCode, LocIDSuffix From " & smBILLTblPath & "Customer where Active = 'Y' order by Name"
            PopulateDataset2(dtAdapter, dtSet, SQLSelName)
        Else
            SQLSelID = "Select CustomerID, Name, Address1, Address2, City, State, Zip, Contact, Phone, CourierCode, LocIDSuffix From " & smBILLTblPath & "Customer where Active = 'Y' order by CustomerID"
            ...
            Finally
                If HasErr = False Then
                    ...
                    If btnSelectName.Focused = True Then
                        ...
                        Dim SqlSelectPlanID As String = "Select c.Name, c.CustomerId, ppc.PlanID From " & smBILLTblPath & "Customer c, " & smBILLTblPath & "PricePlanCustomer ppc where c.CustomerID=ppc.CustomerID"
                    Else
                        utID.Text = ugRow.Cells("CustomerID").Text
                        utName.Text = ugRow.Cells("Name").Text
                    End If
    ...
    Private Sub utID_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles utID.Leave, utName.Leave
        ...
        Else
            If utID.ContainsFocus = True Then
                If SearchOnLeave(sender, sender, smBILLTblPath & "Customer", "CustomerID", "CustomerID", , , "where Active = 'Y'") = False Then
                    ...
                    Exit Sub
                Else
                    If ReturnRowByID(utID.Text, row, smBILLTblPath & "Customer", "", "CustomerID") Then
                        utName.Text = row("Name")
                    End If
                End If
            ElseIf utName.ContainsFocus = True Then
                If SearchOnLeave(sender, sender, smBILLTblPath & "Customer", "Name", "Name") = False Then
                    'MsgBox("Account not found.")
                    utID.Text = ""
                    utName.Text = ""
                    Group_EnDis(False)
                    ...
                    Exit Sub
                Else
                    If ReturnRowByName(utName.Text, row, smBILLTblPath & "Customer", "where Active = 'Y'", "Name") Then
            ...
            If MsgBox("Are you sure that you want to Assign Price Plan(s) to the selected customer?", MsgBoxStyle.YesNo, "Assigning Price Plan(s) to the Customer!") = MsgBoxResult.Yes Then
                For i = 0 To UltraGrid1.Rows.Count - 1
                    If UltraGrid1.Rows(i).Cells("CHK").Value = "1" Then
                        ...
                        InsQry = "Insert into " & smBILLTblPath & "PricePlanCustomer(PlanID, CustomerID) Select " & UltraGrid1.Rows(i).Cells("PlanID").Text & ", '" & utID.Text & "'"
                        ExecuteQuery(InsQry)
                    End If
                Next
            Else
            ...
            If MsgBox("Are you sure that you want to Relieve/Assign Price Plan(s) to the selected customer?", MsgBoxStyle.YesNo, "Assigning Price Plan(s) to the Customer!") = MsgBoxResult.Yes Then
                For i = 0 To UltraGrid2.Rows.Count - 1
                    If UltraGrid2.Rows(i).Cells("CHK").Value = "1" Then
                        ...
                        DelQry = "Delete " & smBILLTblPath & "PricePlanCustomer where PlanID = " & UltraGrid2.Rows(i).Cells("PlanID").Text & " and CustomerID = '" & utID.Text & "'"
                        ExecuteQuery(DelQry)
                    End If
                Next
            Else
    ...
    Private Sub btnExit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExit.Click
        Me.Close()
    End Sub
    Private Sub smPricePlanCustomer_Closing(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles MyBase.Closing
