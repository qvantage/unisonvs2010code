...
Public Class smBillingInvoiceGen
...
#End Region
    ...
    Private Sub smBillingInvoiceGen_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        FillUCombo(ucboTerms, "", "", "Select TermsID as fldCode, Term as fldLabel, Days from " & smBILLTblPath & "InvoiceTerms order by Term")
    ...
    Private Function GetNextInvoiceNo() As String
        Dim qBill As String = "Select * From " & smBILLTblPath & "BillingSetup "
        Dim qMax As String = "Select isnull(Max([Invoice_No]), 0) as MaxNo From " & smBILLTblPath & "Invoices "
    ...
    'Private Function GetNextInvoiceNo() As String
    '    Dim qBill As String = "Select * From " & smBILLTblPath & "BillingSetup "
    '    Dim qMax As String = "Select isnull(Max([Invoice No]), 0) as MaxNo From " & smBILLTblPath & "BillingInvoice "
    '    Dim dsTemp As New System.Data.DataSet
    '    Dim daTemp As New SqlDataAdapter
    '    Dim row As DataRow
    '    Dim StartNo, NextNo, MaxNo As Integer
    ...
    Private Function CreateBlankInvoice(ByVal InvID As Int32, ByVal InvDate As String, ByVal CloseDate As String, ByVal BillInfo As CustBillInfo) As Boolean
        Dim qInsert = "Insert into " & smBILLTblPath & " Invoices(Invoice_NO, Invoice_Date, Due_Date, CustomerID, Name, Contact, Address1, Address2, City, State, Zip, Phone, Fax, email, Closing_Date) " & _
...
Release:
    ...
    Private Function GetBillInfo(ByVal CustID As String) As CustBillInfo
        ...
        Dim q = " Select CustomerID, Name, Contact, Address1, Address2, City, State, Zip, Phone, eMail From " & smBILLTblPath & "Customer " ' where CustomerID = '" & CustID & "'"
        Dim q2 As String = " Select * from " & smBILLTblPath & "InvoiceTerms where TermsID = " & ucboTerms.Value
        Dim row As DataRow
        Dim row2 As DataRow

        If ReturnRowByID(CustID, row, smBILLTblPath & "Customer", "", "CustomerID") Then
            TmpInfo.ID = CustID
    ...
    Private Sub GenInvByAccount()
        ...
        'TODO:  Unimplmented Billing Modules
        ...
        'ExecuteQuery("Update " & smBILLTblPath & "BillingInvoice Set [Amnt Due] = (Select ISNULL(SUM(Charge), 0) as RCharge From " & smBILLTblPath & "BillingInvoiceRouteDetails where [Invoice No] = " & InvID & "), [Curr Bal] = (Select ISNULL(SUM(Charge), 0) as RCharge From " & smBILLTblPath & "BillingInvoiceRouteDetails where [Invoice No] = " & InvID & ") Where [Invoice No] = " & InvID)
        CalcFuelSurcharge(utFuel.Text.Trim, InvID, BillInfo, UltraDate2.Value)

        SetInvoiceTotal(InvID)

        '' Add Peninsula Loop.  
        'If BillInfo.ID = "1103" Then
        ...
        '    ' Execute PeninsulaCoastAdjustments
        '    Dim qChg As String = smBILLTblPath & "PeninsulaCoastAdjustments " & InvID
        '    ExecuteQuery(qChg)
        ...
        InvID += 1
        ' Already done in Create Invoice : ExecuteQuery("Update " & smBILLTblPath & "BillingSetup Set [Next Invoice No] = " & InvID)
        ' Run Query to Total the Invoices created
        Dim sQuery As String = "Select isnull(count(Invoice_No), 0) as InvCount, isnull(Sum(Total_Amount), 0) as Total From " & smBILLTblPath & "Invoices where [Invoice_No] between " & BeginInvID & " AND " & InvID - 1 & ""
        'If Not PopulateDataset2(daAllAccts, dsAllAccts, "Select isnull(count(Invoice_No), 0) as InvCount, isnull(Sum(Total_Amount), 0) as Total From " & smBILLTblPath & "Invoices where [Invoice_No] between " & BeginInvID & " AND " & InvID - 1 & "") Is Nothing Then
...
Release:
    ...
    Private Sub SetInvoiceTotal(ByVal InvId As Int32)
        Dim qSum1 As String = "Update " & smBILLTblPath & "Invoices Set Total_Amount = (Select Sum(isnull(Charge, 0)) as TotalCharge from " & smBILLTblPath & "InvoiceLineItems where invoice_no = " & InvId & ") From " & smBILLTblPath & "Invoices i Where i.Invoice_No =  " & InvId & ""
        If InvId = 0 Then
            'Message modified by Michael Pastor
            MsgBox("Invoice number remains unspecified. Please enter a valid invoice number to continue.", MsgBoxStyle.Exclamation, "Missing Data Input")
            '- MsgBox("Invoice No. is not specified.")
        ...
        ExecuteQuery(qSum1)
    End Sub
    Private Sub GenInvByBillingCycle()
        Dim dsAllAccts As New System.Data.DataSet
        Dim daAllAccts As New SqlDataAdapter
        Dim row As DataRow
        ...
        For Each row In dsAllAccts.Tables(0).Rows
            ...
            InvID += 1
        Next
        ' Already done in CreateInvoice : ExecuteQuery("Update " & smBILLTblPath & "BillingSetup Set [Next Invoice No] = " & InvID)
        ' Run Query to Total the Invoices created
        daAllAccts.Dispose()
        dsAllAccts.Dispose()
        If Not PopulateDataset2(daAllAccts, dsAllAccts, "Select isnull(count([Curr Bal]), 0) as InvCount, isnull(Sum([Curr Bal]), 0) as Total From " & smBILLTblPath & "BillingInvoice where [Invoice No] between " & BeginInvID & " AND " & InvID - 1 & "") Is Nothing Then
            If Not dsAllAccts.Tables Is Nothing Then
                If dsAllAccts.Tables(0).Rows.Count > 0 Then
                    row = dsAllAccts.Tables(0).Rows(0)
                    utTotalAmount.Text = row("Total")
...
Release:
    ...
    Private Function CreateInvoice(ByVal InvoiceNo As String, ByVal AcctID As Integer, ByVal AcctName As String, ByVal InvoiceDate As Date, ByVal PeriodBegin As Date, ByVal PeriodEnd As Date, ByVal BCycleCode As String) As Integer
        Dim qSearchExistingInv As String = "Select * From " & smBILLTblPath & "BillingInvoice where [Acct ID] = " & AcctID & " AND [Fr Date] = '" & PeriodBegin & "' and [To Date] = '" & PeriodEnd & "'"
        Dim qSearchExistingInvNo As String = "Select * From " & smBILLTblPath & "BillingInvoice where [Invoice No] = " & InvoiceNo & ""
        ...
        '    If ExecuteQuery("Insert Into " & smBILLTblPath & "BillingInvoice([Invoice No], [Acct ID], [Acct Name], [Invoice Date], [Fr DATE], [To DATE]) values(" & InvoiceNo & ", " & AcctID & ", '" & Replace(AcctName, "'", "''") & "', '" & InvoiceDate & "', '" & PeriodBegin & "', '" & PeriodEnd & "')") = True Then
        '        CreateInvoice = InvoiceNo
        '        ExecuteQuery("Update " & smBILLTblPath & "BillingSetup Set [Next Invoice No] = " & InvoiceNo + 1 & "")
        '    End If
        '    daTempInv.Dispose()
        '    dsTempInv.Dispose()
...
Release:
    ...
    Private Function AssignInvoice(ByVal InvoiceNo As String, ByVal AcctID As String, ByVal InvoiceDate As Date)
        ExecuteQuery("Update " & smBILLTblPath & "BillingInvoiceRouteDetails set [Invoice Date] = '" & InvoiceDate & "', [Invoice No] = " & InvoiceNo & " where [Acct ID] = " & AcctID & " AND [Invoice No] = 0")
        ' For Other Modules ... ExecuteQuery("Update " & smBILLTblPath & "BillingInvoiceRouteDetails set [Invoice Date] = '" & InvoiceDate & "', [Invoice No] = " & InvoiceNo & " where [Acct ID] = " & AcctID & " AND [Invoice No] = 0")
...
NextService:
    ...
    Private Function PricePlanBilling(ByVal TempInvID As Integer, ByVal AcctID As String, ByVal BCycleCode As String, ByVal PeriodBegin As Date, ByVal PeriodEnd As Date)
        ...
        Dim qPlans As String = "Select pp.PlanID, pp.Plan_Name, pp.PlanTypeCode, pp.Charge_Code, pp.From_Zone, pp.To_Zone, pp.Start_Date, pp.End_Date, pp.ModuleName, pp.TableName, pp.ColumnName, pp.ColumnPrefix, pp.ColumnSuffix, pp.Invoice_Title, pp.Taxable, pp.Description From " & smBILLTblPath & "PricePlans pp, " & smBILLTblPath & "PricePlanCustomer ppc where pp.planid = ppc.planid and ppc.CustomerID = '" & AcctID & "' Order by pp.PlanID"
        Dim dsTemp As New System.Data.DataSet
        Dim daTemp As New SqlDataAdapter
        Dim row As DataRow
        Dim ActualPeriodEnd, ActualPeriodBegin As Date
    ...
    Private Sub PPCalcRangeCharge(ByVal FromCustID As String, ByVal TempInvID As Integer, ByVal InvCloseDate As Date, ByVal PlanID As Integer, ByVal StartDate As Date, ByVal EndDate As Date, ByVal ModuleName As String, ByVal TableName As String, ByVal ColumnName As String, ByVal ColPrefix As String, ByVal ColSuffix As String, ByVal ChargeTitle As String, ByVal Desc As String, ByVal Taxable As Boolean, Optional ByVal FromZone As Int32 = 0, Optional ByVal ToZone As Int32 = 0)
        ...
        If ModuleName.ToUpper = "TRACKING" Then
            'Dim qChg As String = smBILLTblPath & "CalcMftCharge_RangeV2 " & PlanID & ", '" & TempInvID & "', '" & FromCustID & "', '" & StartDate & "', '" & IIf(ToDate = Nothing, "", ToDate) & "', " & FromZone & ", " & ToZone & ", '" & TableName.Trim & "', '" & ColumnName.Trim & "', '" & smBILLTblPath & "' "
            Dim qChg As String = smBILLTblPath & "CalcMftCharge_RangeV3 " & PlanID & ", '" & TempInvID & "', '" & FromCustID & "', '" & StartDate & "', '" & IIf(ToDate = Nothing, "", ToDate) & "', " & FromZone & ", " & ToZone & ", '" & TableName.Trim & "', '" & ColumnName.Trim & "', '" & smBILLTblPath & "' "
            ExecuteQuery(qChg)
            ...
            '                          " Select mftx.Invoice_NO , (Select TOP 1 Invoice_Date from " & AppTblPath & "Invoices where invoice_No = mftx.Invoice_No ORDER BY INVOICE_dATE DESC) as Invoice_Date, '" & Desc & "' as [Description], NULL as UnitPrice, '" & ColPrefix & "' as Prefix, mftx." & ColumnName & " as Qty, '' as suffix, '" & ColSuffix & "' as unit, Charge , '" & IIf(Taxable, "T", "") & "' as Tax, " & PlanID & " as PlanID, mftx.RowID from " & AppTblPath & "SettlementInvoice mftx where mftx.invoice_no = '" & TempInvID & "' and mftx.PlanID = " & PlanID
        ...
        'Select mftx.Invoice_NO, (Select isnull(max(LineNum), 0)+1 from InvoiceLineItems where Invoice_No = mftx.Invoice_No ) as LineNum, (Select TOP 1 Invoice_Date from Invoices where invoice_No = mftx.Invoice_No ORDER BY INVOICE_dATE DESC) as Invoice_Date, 'Desc' as [Description], 5.6 as UnitPrice, '' as Prefix, weight as Qty, '' as suffix, 'lb' as unit, Charge from SettlementInvoice mftx where mftx.invoice_no = 123
    ...
    Private Sub PPCalcFixedCharge(ByVal FromCustID As String, ByVal TempInvID As Integer, ByVal InvCloseDate As Date, ByVal PlanID As Integer, ByVal StartDate As Date, ByVal EndDate As Date, ByVal ModuleName As String, ByVal TableName As String, ByVal ColumnName As String, ByVal ColPrefix As String, ByVal ColSuffix As String, ByVal ChargeTitle As String, ByVal Desc As String, ByVal Taxable As Boolean, Optional ByVal FromZone As Int32 = 0, Optional ByVal ToZone As Int32 = 0)
        ...
        If ModuleName.ToUpper = "TRACKING" Then
            If ColumnName.Trim <> "" Then
                'Dim qChg As String = "CalcMftCharge_FixedV2 " & PlanID & ", '" & TempInvID & "', '" & FromCustID & "', '" & StartDate & "', '" & IIf(ToDate = Nothing, "", ToDate) & "', " & FromZone & ", " & ToZone & ", '" & ColumnName.Trim & "', '" & smBILLTblPath & "' "
                Dim qChg As String = smBILLTblPath & "CalcMftCharge_FixedV3 " & PlanID & ", '" & TempInvID & "', '" & FromCustID & "', '" & StartDate & "', '" & IIf(ToDate = Nothing, "", ToDate) & "', " & FromZone & ", " & ToZone & ", '" & TableName.Trim & "', '" & ColumnName.Trim & "', '" & smBILLTblPath & "' "
                ExecuteQuery(qChg)
            End If
            ...
            '                          " Select mftx.Invoice_NO , (Select TOP 1 Invoice_Date from " & AppTblPath & "Invoices where invoice_No = mftx.Invoice_No ORDER BY INVOICE_dATE DESC) as Invoice_Date, '" & Desc & "' as [Description], NULL as UnitPrice, '" & ColPrefix & "' as Prefix, mftx." & ColumnName & " as Qty, '' as suffix, '" & ColSuffix & "' as unit, Charge , '" & IIf(Taxable, "T", "") & "' as Tax, " & PlanID & " as PlanID from " & AppTblPath & "SettlementInvoice mftx where mftx.invoice_no = '" & TempInvID & "' and mftx.PlanID = " & PlanID
            'If ChargeTitle.Trim <> "" Then
            '    'Dim qTitle As String = "Insert into  " & AppTblPath & "InvoiceLineItems(Invoice_No, Invoice_Date, Description, PlanID) " & _
            '    '                      " Select mftx.Invoice_NO , (Select isnull(max(LineNum), 0)+1 from " & AppTblPath & "InvoiceLineItems where Invoice_No = mftx.Invoice_No ) as LineNum, (Select TOP 1 Invoice_Date from " & AppTblPath & "Invoices where invoice_No = mftx.Invoice_No ORDER BY INVOICE_dATE DESC) as Invoice_Date, '" & ChargeTitle & "' as [Description], NULL as UnitPrice, '' as Prefix, NULL as Qty, '' as suffix, '' as unit, 0 as Charge , '' as Tax, " & PlanID & " as PlanID from " & AppTblPath & "SettlementInvoice mftx where mftx.invoice_no = '" & TempInvID & "' and mftx.PlanID = " & PlanID
            '    Dim qTitle As String = "Insert into  " & AppTblPath & "InvoiceLineItems(Invoice_No, Invoice_Date, Description, PlanID) " & _
            '                           " Select '" & TempInvID & "' as Invoice_NO , (Select TOP 1 Invoice_Date from " & AppTblPath & "Invoices where invoice_No = '" & TempInvID & "' ORDER BY INVOICE_dATE DESC) as Invoice_Date, '" & ChargeTitle & "' as [Description], " & PlanID & " as PlanID "
...
Release:
    ...
    Private Sub MiscBilling(ByVal TempInvID As Int32, ByVal billinginfo As CustBillInfo, ByVal PeriodEnd As Date)
        Dim q As String = "" & _
" Insert into " & smBILLTblPath & "InvoiceLineItems(Invoice_No, Invoice_Date, TranDate, Charge_Code, Description, UnitPrice, Prefix, Qty, Suffix, Unit, Charge, Tax, MISCRowID )  " & _
" SELECT     '" & TempInvID & "' as Invoice_No, (Select TOP 1 Invoice_Date from " & smBILLTblPath & "Invoices where invoice_No = '" & TempInvID & "' ORDER BY INVOICE_dATE DESC) as Invoice_Date, imc.Trandate as TranDate, imc.Charge_Code, imc.Description, " & _
...
" FROM  " & smBILLTblPath & "InvoiceMiscCharges imc " & _
" Where imc.BillToCustID = '" & billinginfo.ID & "' and imc.Invoice_No is NULL " & _
" AND imc.TranDate < dateadd(day, 1, '" & PeriodEnd & "') ;"

        Dim q2 = " Update " & smBILLTblPath & "InvoiceMiscCharges Set Invoice_No = '" & TempInvID & "' " & _
                 " FROM " & smBILLTblPath & "InvoiceMiscCharges imc Where imc.BillToCustID = '" & billinginfo.ID & "' and imc.Invoice_No is NULL " & _
                 " AND imc.TranDate < dateadd(day, 1, '" & PeriodEnd & "') ;"
    ...
    '        Dim q As String = "" & _
    '" Insert into " & smBILLTblPath & "InvoiceLineItems(Invoice_No, Invoice_Date, TranDate, Charge_Code, Description, UnitPrice, Prefix, Qty, Suffix, Unit, Charge, Tax, MISCRowID )  " & _
    '" SELECT     '" & TmpInvId & "' as Invoice_No, (Select TOP 1 Invoice_Date from " & smBILLTblPath & "Invoices where invoice_No = '" & TmpInvId & "' ORDER BY INVOICE_dATE DESC) as Invoice_Date, imc.Trandate as TranDate, imc.Charge_Code, imc.Description, " & _
    '" NULL as UnitPrice, '' as Prefix, imc.qty as Qty, '' as suffix, imc.Unit as Unit, imc.charge as Charge,  (case imc.taxable when 1 then 'T' else '' end) as tax, imc.rowid " & _
    '" FROM  " & smBILLTblPath & "InvoiceMiscCharges imc " & _
    '" Where imc.BillToCustID = '" & billingInfo.ID & "' and imc.Invoice_No is NULL " & _
    '" AND imc.TranDate < dateadd(day, 1, '" & PeriodEnd & "') ;"
    ...
    Private Sub CalcFuelSurcharge(ByVal RatePct As Decimal, ByVal TempInvID As Int32, ByVal billinginfo As CustBillInfo, ByVal PeriodEnd As Date)
        '        Dim q As String = "" & _
        '" Insert into " & smBILLTblPath & "InvoiceMiscCharges(Invoice_No, TranDate, BillToCustID, BillToCustName, Charge_Code, Description, Qty, Unit, Charge) " & _
        ...
        '", (Select Sum(Charge) from " & smBILLTblPath & "Invoices where Invoice_No = '" & TempInvID & "') * " & RatePct & "/100. as Charge "

        If RatePct = 0 Then Exit Sub

        Dim q As String = "" & _
" Insert into " & smBILLTblPath & "InvoiceLineItems(Invoice_No, Invoice_Date, Charge_Code, Description, UnitPrice, Prefix, Qty, Suffix, Unit, Charge, Tax, MISCRowID )  " & _
" SELECT     '" & TempInvID & "' as Invoice_No " & _
", (Select TOP 1 Invoice_Date from " & smBILLTblPath & "Invoices where invoice_No = '" & TempInvID & "' ORDER BY INVOICE_dATE DESC) as Invoice_Date" & _
", 'FUE' as Charge_Code, 'Fuel Surcharge' as Description, " & _
" NULL as UnitPrice, '' as Prefix, " & RatePct & "/100. as Qty, '%%' as suffix, '' as Unit" & _
", (Select Sum(Charge) from " & smBILLTblPath & "InvoiceLineItems where Invoice_No = '" & TempInvID & "') * " & RatePct & "/100. as Charge" & _
",  '' as tax, NULL "
    ...
    Private Sub MoveMftInvToArchive(ByVal TempInvID As Int32)
        Dim q As String = " Insert into " & TRCTblPath & "SettlementInvoiceArchive Select * from " & TRCTblPath & "SettlementInvoice where Invoice_No = '" & TempInvID & "'"

        Dim q2 As String = "Delete From " & TRCTblPath & "SettlementInvoice where Invoice_No =  '" & TempInvID & "'"
        If ExecuteQuery(q) Then
            ExecuteQuery(q2)
        End If
